...
class SmsReceiver : BroadcastReceiver() {
    ...
    private fun parseExpenseMessage(message: String, sender: String): ExpenseData? {
        ...
        if (isIncomingMoney(msg)) {
            Log.d(TAG, "Skipping incoming money message")
            return null
        }

        // SECOND: Filter OUT EMI/Bill reminders
        if (isReminderNotification(msg)) {
            Log.d(TAG, "Skipping reminder notification")
            return null
        }
    ...
    private fun isIncomingMoney(msg: String): Boolean {
    ...
    private fun isReminderNotification(msg: String): Boolean {
    ...
    private fun extractReceiver(message: String, sender: String): String? {
        ...
        val patterns = listOf(
            "TO\\s+([A-Z][A-Z\\s]+)".toRegex(),
            "PAID\\s+TO\\s+([A-Z][A-Z0-9@]+)".toRegex(),
            "AT\\s+([A-Z][A-Z0-9\\s]+)".toRegex(),
            "SENT\\s+TO\\s+([A-Z][A-Z\\s]+)".toRegex()
        )
    ...
    private fun identifySource(sender: String, message: String): String {
        ...
        return when {
            ...
            senderUpper.contains("SBI") || msgUpper.contains("STATE BANK") -> "SBI"
            ...
            senderUpper.contains("GPAY") || msgUpper.contains("GOOGLE PAY") -> "Google Pay"
            else -> "Bank/Unknown"
        }
    }
        ...
        try {
            ...
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS expenses (
                    id TEXT PRIMARY KEY, amount REAL NOT NULL, description TEXT NOT NULL,
                    category TEXT NOT NULL, paymentModeId TEXT, createdAt TEXT NOT NULL, rawSms TEXT
                )
            """.trimIndent())
            ...
            db.execSQL("""
                CREATE TABLE IF NOT EXISTS payment_modes (
                    ...
                    lastFourDigits TEXT, createdAt TEXT NOT NULL
                )
            """.trimIndent())
    ...
    private fun showExpenseSavedNotification(context: Context, expenseData: ExpenseData) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channel = NotificationChannel(CHANNEL_ID, "Expense Detection", NotificationManager.IMPORTANCE_DEFAULT)
            channel.description = "Notifications for detected expenses from SMS"
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setStyle(NotificationCompat.BigTextStyle().bigText("$amountStr$receiverText from ${expenseData.source}\n\nAuto-saved to your expenses."))
